...
CREATE INDEX IF NOT EXISTS idx_conversations_created ON conversations(phone_number, created_at DESC);
...
-- Anon key can read users (for checking tier from API)
CREATE POLICY "Anon can read users" ON users
    FOR SELECT
    USING (true);

-- Anon key can insert usage (for logging from API)
CREATE POLICY "Anon can insert usage" ON usage
    FOR INSERT
    WITH CHECK (true);

-- Anon key can manage conversations
...
COMMENT ON TABLE conversations IS 'Message history for AI context';
